...
# Ecosystem
* [@shapes/dynamodb](packages/@shapes/dynamodb) - DSL for working with AWS DynamoDB. Supports type-safe Query, Update and Conditional expressions, and maps between the raw attribute values and the shape definition.
* [@shapes/hive](packages/@shapes/hive) - maps Shapes to AWS Glue (Hive) schemas to support declaring Hive/Glue tables with a Struct.
* [@shapes/json](packages/@shapes/json) - JSON serialization.
* [@shapes/jsonpath](packages/@shapes/jsonpath) - Type-safe DSL for constructing JSON path expressions from Shapes.
* [@shapes/jsonschema](packages/@shapes/jsonschema) - Maps a Shape to its corresponding JSON schema.
