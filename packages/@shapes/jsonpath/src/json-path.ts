import { AnyShape, array, ArrayShape, BinaryShape, bool, BoolShape, EnumShape, FunctionArgs, FunctionShape, IntegerShape, LiteralShape, MapShape, NeverShape, NothingShape, number, NumberShape, SetShape, Shape, ShapeGuards, ShapeVisitor, string, StringShape, TimestampShape, StructShape, UnionShape, Value } from '@shapes/core';
...
export namespace JsonPath {
  ...
  export const Tag = Symbol.for('@shapes/jsonpath.JsonPath.Tag');
  ...
  export type Of<T extends Shape> =
    ...
    T extends UnionShape<infer U> ?
      ...
      NothingShape extends Extract<U[Extract<keyof U, number>], NothingShape> ?
        U['length'] extends 1 ? JsonPath.Thing<NothingShape> :
        U['length'] extends 2 ?
          Exclude<{
            [i in Extract<keyof U, number>]: Of<U[i]>;
          }[Extract<keyof U, number>], JsonPath.Thing<NothingShape>> :
        JsonPath.Union<T> :
      JsonPath.Union<T> :
  ...
  export type Root<T extends StructShape> = Struct<T>[Fields];
  ...
  export const NodeType = Symbol.for('@shapes/jsonpath.JsonPath.NodeType');
  export const SubNodeType = Symbol.for('@shapes/jsonpath.JsonPath.SubNodeType');
  export const DataType = Symbol.for('@shapes/jsonpath.JsonPath.DataType');
  export const InstanceExpression = Symbol.for('@shapes/jsonpath.JsonPath.InstanceExpression');
  export const Synthesize = Symbol.for('@shapes/jsonpath.JsonPath.Synthesize');
  export const ExpressionTag = Symbol.for('@shapes/jsonpath.JsonPath.ExpressionTag');
  ...
  export class Thing<T extends Shape = Shape> extends ExpressionNode<T> {
  ...
  export namespace Thing {
    ...
    export class NotEquals<T extends Shape> extends Thing.Comparison<T, T> {
      protected readonly operator: '!=' = '!=';
  ...
  export class Number extends Thing<Number.Shape> {
    ...
    public equals(other: Expression<Number.Shape>): Bool {
      return new Bool(new Thing.Equals(this, resolveExpression(number, other as any)));
    }
    public notEquals(other: Expression<Number.Shape>): Bool {
      return new Bool(new Thing.NotEquals(this, resolveExpression(number, other as any)));
    }
  }
  export namespace Number {
    export type Shape = NumberShape | IntegerShape;

    export class Gt<T extends Shape> extends Thing.Comparison<T, Number.Shape> {
      protected readonly operator: '>' = '>';
      public readonly [SubNodeType] = 'greaterThan';
    }
    export class Gte<T extends Shape> extends Thing.Comparison<T, Number.Shape> {
      protected readonly operator: '>=' = '>=';
      public readonly [SubNodeType] = 'greaterThanOrEqual';
    }
    export class Lt<T extends Shape> extends Thing.Comparison<T, Number.Shape> {
      protected readonly operator: '<' = '<';
      public readonly [SubNodeType] = 'lessThan';
    }
    export class Lte<T extends Shape> extends Thing.Comparison<T, Number.Shape> {
      protected readonly operator: '<=' = '<=';
      public readonly [SubNodeType] = 'lessThanOrEqual';
    }
  }

  export class String extends Thing<StringShape> {
    public equals(other: Expression<StringShape>): Bool {
      return new Bool(new Thing.Equals(this, resolveExpression(string, other as any)));
    }
    public notEquals(other: Expression<StringShape>): Bool {
      return new Bool(new Thing.NotEquals(this, resolveExpression(string, other as any)));
    }
  ...
  export const Item = Symbol.for('@shapes/jsonpath.JsonPath.Item');

  export class Array<T extends Shape> extends Thing<ArrayShape<T>> {
  ...
  export const Fields = Symbol.for('@shapes/jsonpath.JsonPath.Fields');
...
class Visitor implements ShapeVisitor<any, JsonPath.ExpressionNode<any>> {
  ...
  public nothingShape(shape: NothingShape, expression: JsonPath.ExpressionNode<any>): JsonPath.Thing<NothingShape> {
    return new JsonPath.Thing(shape, expression);
  }
  ...
  public structShape(shape: StructShape<any>, expression: JsonPath.ExpressionNode<any>): JsonPath.Struct<StructShape<any>> {
