import { ArrayShape, MapShape, SetShape, Shape, AnyShape, BinaryShape, bool, BoolShape, NothingShape, number, NumberShape, string, StringShape, timestamp, TimestampShape, EnumShape, ShapeGuards, ShapeVisitor, StructShape, UnionShape, Value } from '@shapes/core';
...
export namespace DSL {
  ...
  export const Tag = Symbol.for('@shapes/dynamodb.Query.Tag');
    ...
    */
    ...
    T extends UnionShape<infer U> ?
      ...
      NothingShape extends Extract<U[Extract<keyof U, number>], NothingShape> ?
        U['length'] extends 1 ? DSL.Thing<NothingShape> :
        U['length'] extends 2 ?
          Exclude<{
            [i in Extract<keyof U, number>]: Of<U[i]>;
          }[Extract<keyof U, number>], DSL.Thing<NothingShape>> :
        DSL.Union<T> :
      DSL.Union<T> :

    T extends EnumShape ? Enum<T> :
    T extends StructShape<any> ? DSL.Struct<T> :
  ...
  export type Root<T extends StructShape<any>> = Struct<T>['fields'];
  ...
  export const DslVisitor: ShapeVisitor<Node, ExpressionNode<any>> = {
    ...
    nothingShape: (shape: NothingShape, expression: ExpressionNode<any>): Thing<NothingShape> => {
      return new Thing(shape, expression);
    },
    ...
    structShape: (shape: StructShape<any>, expression: ExpressionNode<any>): Struct<any> => {
...
export namespace DSL {
  ...
  export const NodeType = Symbol.for('@shapes/dynamodb.DSL.NodeType');
  export const SubNodeType = Symbol.for('@shapes/dynamodb.DSL.SubNodeType');
  export const DataType = Symbol.for('@shapes/dynamodb.DSL.DataType');
  export const InstanceExpression = Symbol.for('@shapes/dynamodb.DSL.InstanceExpression');
  export const Synthesize = Symbol.for('@shapes/dynamodb.DSL.Synthesize');
  ...
  export class Thing<T extends Shape> extends ExpressionNode<T> {
    ...
    public equals(other: Expression<T>): Bool {
      return new Bool(new Thing.Equals(this, resolveExpression(this[DataType], other)));
    }
    ...
    public set(value: Expression<T> | Computation<T>): Action {
      return new Action(ActionType.SET, new Thing.Assign(this, resolveExpression(this[DataType], value)));
    }
  ...
  export namespace Thing {
    ...
    export class Assign<T extends Shape> extends StatementNode {
      ...
      constructor(private readonly instance: Thing<T>, private readonly value: Node) {
        super();
      }
    ...
    export class Equals<T extends Shape> extends Thing.Comparison<T, T> {
      protected readonly operator: '=' = '=';
      public readonly [SubNodeType] = 'equals';
    }
  }

  export function size(path: Thing<any>) {
    return new Size(path);
  }
  export class Size extends FunctionCall<NumberShape> {
    constructor(path: Thing<any>) {
      super('size', number, [path]);
    }
  }

  export class Bool extends Thing<BoolShape> {
  ...
  export class Ord<T extends Shape> extends Thing<T> {
  ...
  export namespace Ord {
    export class Gt<T extends Shape> extends Thing.Comparison<T, NumberShape> {
      protected readonly operator: '>' = '>';
      public readonly [SubNodeType] = 'greaterThan';
    }
    export class Gte<T extends Shape> extends Thing.Comparison<T, NumberShape> {
      protected readonly operator: '>=' = '>=';
      public readonly [SubNodeType] = 'greaterThanOrEqual';
    }
    export class Lt<T extends Shape> extends Thing.Comparison<T, NumberShape> {
      protected readonly operator: '<' = '<';
      public readonly [SubNodeType] = 'lessThan';
    }
    export class Lte<T extends Shape> extends Thing.Comparison<T, NumberShape> {
      protected readonly operator: '<=' = '<=';
  ...
  export class Binary extends Thing<BinaryShape> {}
  ...
  export class Timestamp extends Thing<TimestampShape> {
  ...
  export class List<T extends Shape> extends Thing<ArrayShape<T>> {
    ...
    public push(item: Expression<T>): Action {
      return new Action(ActionType.SET, new Thing.Assign(this.get(1) as any, resolveExpression(this[DataType].Items, item)));
    }

    public concat(list: Expression<ArrayShape<T>>): Action {
      return new Action(ActionType.SET, new Thing.Assign(this, new List.Append(this, resolveExpression(this[DataType], list) as List<T>)));
    }
  }
  ...
  export class Set<T extends Shape> extends Thing<SetShape<T>> {
  ...
  export class Map<T extends Shape> extends Thing<MapShape<T>> {
    ...
    public put(key: Expression<StringShape>, value: Expression<T>): Action {
      return new Action(ActionType.SET, new Thing.Assign(this.get(key) as any, resolveExpression(this[DataType].Items, value)));
    }
  }
  ...
  export class Any<T extends AnyShape> extends Thing<T> {
  ...
  export class Union<T extends UnionShape<Shape[]>> extends Thing<T> {
