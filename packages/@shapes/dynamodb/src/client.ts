...
import { TypeShape, Value } from '@shapes/core';
...
export interface BaseClientProps<T extends TypeShape, K extends DDB.KeyOf<T>> {
  /**
...
export class BaseClient<T extends TypeShape<any>, K extends DDB.KeyOf<T>> {
...
export interface TableClientProps<T extends TypeShape, K extends DDB.KeyOf<T>> extends Omit<BaseClientProps<T, K>, 'indexName'> {}

export class TableClient<T extends TypeShape, K extends DDB.KeyOf<T>> extends BaseClient<T, K> {
...
export interface IndexClientProps<T extends TypeShape<any>, K extends DDB.KeyOf<T>> extends TableClientProps<T, K> {
  /**
   * Name of the DynamoDB Index.
 ...
 */
export class IndexClient<T extends TypeShape, K extends DDB.KeyOf<T>> extends BaseClient<T, K> {
...
export namespace DDB {
  type _QueryOutput<T extends TypeShape<any>, K extends KeyOf<T>> = Compact<
    Omit<AWS.DynamoDB.QueryOutput, 'Items' | 'LastEvaulatedKey'> & {
      Items?: Value.Of<T>[];
      LastEvaluatedKey?: KeyValue<T, K>
    }>;
  export interface QueryOutput<T extends TypeShape<any>, K extends KeyOf<T>> extends _QueryOutput<T, K> {}

  export type HashKey<T> = keyof T;
  export type SortKey<T> = [keyof T, keyof T];
  export interface KeyOf<T extends TypeShape> {
    partition: keyof T['Members'];
    sort?: keyof T['Members'] | undefined;
  }

  export type KeyNames<T extends TypeShape<any>, K extends KeyOf<T>> =
    K extends { partition: infer PK, sort?: undefined } ? PK :
    K extends { partition: infer PK, sort: infer SK } ? PK | SK :
    never;

  export type KeyValue<T extends TypeShape<any>, K extends KeyOf<T>> = {
    [k in KeyNames<T, K>]: Value.Of<T['Members'][k]>;
  };

  export type HashKeyName<K> = K extends { partition: infer H; } ? H : never;

  export type HashKeyValue<T extends TypeShape<any>, K extends KeyOf<T>> = Value.Of<HashKeyShape<T, K>>;
  export type HashKeyShape<T extends TypeShape<any>, K extends KeyOf<T>> = T['Members'][HashKeyName<K>];

  export type SortKeyName<K> = K extends { sort?: infer S; } ? S : undefined;

  export type SortKeyValue<T extends TypeShape<any>, K extends KeyOf<T>> = Value.Of<SortKeyShape<T, K>>;
  export type SortKeyShape<T extends TypeShape<any>, K extends KeyOf<T>> = T['Members'][SortKeyName<K>];

  export type QueryCondition<T extends TypeShape<any>, K extends KeyOf<T>> =
    ...
    ;
  export interface QueryProps<T extends TypeShape<any>, K extends KeyOf<T>> {
  ...
  export type Condition<T extends TypeShape<any>> = (item: DSL.Root<T>) => DSL.Bool;
  export interface Update<T extends TypeShape<any>> {
