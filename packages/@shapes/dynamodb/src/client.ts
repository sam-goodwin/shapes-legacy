...
import { StructShape, Value } from '@shapes/core';
...
export interface BaseClientProps<T extends StructShape, K extends DDB.KeyOf<T>> {
  /**
...
export class BaseClient<T extends StructShape<any>, K extends DDB.KeyOf<T>> {
...
export interface TableClientProps<T extends StructShape, K extends DDB.KeyOf<T>> extends Omit<BaseClientProps<T, K>, 'indexName'> {}

export class TableClient<T extends StructShape, K extends DDB.KeyOf<T>> extends BaseClient<T, K> {
...
export interface IndexClientProps<T extends StructShape<any>, K extends DDB.KeyOf<T>> extends TableClientProps<T, K> {
  /**
   * Name of the DynamoDB Index.
 ...
 */
export class IndexClient<T extends StructShape, K extends DDB.KeyOf<T>> extends BaseClient<T, K> {
...
export namespace DDB {
  type _QueryOutput<T extends StructShape<any>, K extends KeyOf<T>> = Compact<
    Omit<AWS.DynamoDB.QueryOutput, 'Items' | 'LastEvaulatedKey'> & {
      Items?: Value.Of<T>[];
      LastEvaluatedKey?: KeyValue<T, K>
    }>;
  export interface QueryOutput<T extends StructShape<any>, K extends KeyOf<T>> extends _QueryOutput<T, K> {}
  ...
  export type QueryCondition<T extends StructShape<any>, K extends KeyOf<T>> =
    ...
    ;
  export interface QueryProps<T extends StructShape<any>, K extends KeyOf<T>> {
  ...
  export type Condition<T extends StructShape<any>> = (item: DSL.Root<T>) => DSL.Bool;
  export interface Update<T extends StructShape<any>> {
